...
def sign_key(flags, ExportedSessionKey, Mode):
	key = b""

	if flags.dict["NEGOTIATE_EXTENDED_SESSIONSECURITY"]:
		if Mode == "Client":
			key = md5(ExportedSessionKey + b"session key to client-to-server signing key magic constant")
		else:
			key = md5(ExportedSessionKey + b"session key to server-to-client signing key magic constant")

	return key

def seal_key(flags, ExportedSessionKey, NTLMRevisionCurrent, Mode):
	if flags.dict["NEGOTIATE_EXTENDED_SESSIONSECURITY"]:
		...
		if Mode == "Client":
			key = md5(key + b"session key to client-to-server sealing key magic constant")
		else:
			key = md5(key + b"session key to server-to-client sealing key magic constant")
		...
		if flags.dict["NEGOTIATE_56"]:
			key = ExportedSessionKey[:6] + b"\xA0"
		else:
			key = ExportedSessionKey[:4] + b"\xE5\x38\xB0"
	else:
		key = ExportedSessionKey
